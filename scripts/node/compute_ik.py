...
class ComputeIK:
    ...
    def inverse_kinematics(self, x, y, z, wx=0.0, wy=-1.0, wz=0.1, ww=0.0):
        """
        ...
        Args:
            ...
            y: y 座標
            z: z 座標
            wx: x 軸の回転 (Optional: default: 0.0)
            wy: y 軸の回転 (Optional: default: -1.0)
            wz: z 軸の回転 (Optional: default: 0.1)
            ww: 回転の角度 (Optional: default: 0.0)
        Returns:
            関節角度のリスト
        """
        rospy.loginfo("Computing IK for coordinates: x=%f, y=%f, z=%f, wx=%f, wy=%f, wz=%f, ww=%f", x, y, z, wx, wy, wz, ww)
        ...
        ik_request.ik_request.pose_stamped.pose.orientation.x = wx
        ik_request.ik_request.pose_stamped.pose.orientation.y = wy
        ik_request.ik_request.pose_stamped.pose.orientation.z = wz
        ik_request.ik_request.pose_stamped.pose.orientation.w = ww
